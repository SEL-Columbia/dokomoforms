...
if [ -z "`awk /TEST_USER/ dokomoforms/local_settings.py`" ]; 
then 
    echo 'TEST_USER = "poop"' >> dokomoforms/local_settings.py; 
else
    sed -i 's/TEST_USER.*/TEST_USER="poop"/' dokomoforms/local_settings.py
fi
