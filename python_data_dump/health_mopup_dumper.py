...
def main():
    ...
    cursor.execute('delete from question_variety;')
    ...
    cursor.execute('delete from answer_choice;')
    ...
    query = "insert into type_constraint (type_constraint_name) values ('integer'), ('multiple_choice');"
    cursor.execute(query)

    query =  "insert into question (title, sequence_number, type_constraint_name, survey_id) values "
    ...
    query += "('yes', 1, '{}', false, 'multiple_choice', 0, '{}'),".format(question_ids[0], survey_id)
    query += "('no',  2, '{}', false, 'multiple_choice', 0, '{}'),".format(question_ids[0], survey_id)
    query += "('dk',  3, '{}', false, 'multiple_choice', 0, '{}');".format(question_ids[0], survey_id)
    cursor.execute(query)
